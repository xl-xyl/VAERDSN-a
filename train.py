...
for epoch in range(n_epoch):
    ...
    while i < len_dataloader:
        ...
        domain_label = torch.ones(batch_size*field_size)
        ...
        class_label = torch.LongTensor(batch_size)
        domain_label = torch.zeros(batch_size*field_size)
        domain_label = domain_label.long()
        ...
        # loss+=kl_div1
        # loss+=kl_div2
        scores = compute_scores(source_class_label, getattr(data_source, label_attr))
    ...
    torch.save(my_net.state_dict(), '/home/learn/VAERDSN/' + 'dsn_mnist_mnistm_epoch_' + str(epoch) + '.pth')
